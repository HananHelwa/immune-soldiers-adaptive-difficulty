...
\title{\textbf{Knowledge Tracing and Adaptive Difficulty in a Serious Game for Immune System Education: Design and Simulation-Based Evaluation}}
...
\begin{abstract}
\noindent
Serious games for immunology education typically apply a fixed level and difficulty progression regardless of what an individual learner has understood, including our own prior game, Immune Soldiers. This paper presents an artificial intelligence layer coupling per-learner Bayesian knowledge tracing over five immunology concepts with a bandit-based adaptive difficulty controller. As the game client is only partially implemented, the system is evaluated in simulation: 200 synthetic learners with heterogeneous skill, learning, and forgetting rates played 120 encounters under the original fixed progression, a random-difficulty ablation, and full adaptation. The original progression leaves one concept, mismatched to a permanently high difficulty, severely under-mastered; the adaptive system equalizes final mastery across all five. A well-tuned fixed difficulty, tested separately, closes most of this gap alone and matches the adaptive system's mastery and calibration; the adaptive system's remaining advantage is lower frustration and a smaller residual equity gap from personalizing to each learner, not higher achievement overall. The ablation isolates difficulty-tier adaptation, not concept selection alone, as the source of that remaining advantage; even so, the system still declares roughly one in six concepts mastered prematurely. Estimates correlate moderately with true mastery, indicating calibration against real classroom data is needed before deployment.
\end{abstract}
...
\begin{itemize}
\item \textbf{An AI layer design for immunology serious games}, coupling a two-channel knowledge tracing model with an adaptive difficulty controller, specified in enough architectural detail (Section 3) to be integrated into an existing Unity-based game once its client-side implementation is complete.
