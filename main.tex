...
A small number of recent serious-game studies have coupled AI-driven adaptation directly to measured learning outcomes rather than only to raw performance -- for example, AI-generated verbal and visual scaffolding delivered by a non-player character in a physics-themed serious game, shown to affect both learning gains and cognitive load \cite{ref10}; and, more directly relevant here, the GRADES framework, which couples a reinforcement-learning DDA controller with a general ``stealth assessment'' signal in collaborative game-based learning \cite{ref40}. It is not established from GRADES' published abstract whether that stealth-assessment signal tracks mastery per individual concept or as a single holistic score per player or group -- we were unable to confirm this from the full text -- but its adaptation target is overall difficulty rather than \textit{which concept to practice next}, so it does not have a mechanism equivalent to the concept-selection component evaluated in this paper (Section 3.3), and cannot exhibit or correct the kind of single-concept ``difficulty trap'' reported in Section 4.3 regardless of that detail.
