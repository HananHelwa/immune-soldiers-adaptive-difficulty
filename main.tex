...
A recurring limitation across serious games generally is that ``AI'' in the game-design sense refers to scripted, non-adaptive enemy behavior -- finite-state agents that pursue or attack the player within a fixed range -- rather than AI in the AIED sense of a model that represents what the learner currently understands and adapts the experience accordingly. Content in this class of game is generally authored once and applied uniformly to every player, which the wider games-adaptivity literature has identified as a persistent source of predictable, impersonal gameplay and limited real-time instructional adaptation \cite{ref3,ref24}. Difficulty typically increases on a fixed schedule (e.g., stage 1 $\to$ stage 2 $\to$ boss) regardless of whether the individual learner has actually grasped the underlying concept that stage was designed to teach. As a result, two learners who finish the same level may leave with very different levels of conceptual mastery, and the game has no way of knowing this, let alone responding to it. This is precisely the gap the AIED literature on learner modeling, knowledge tracing, and dynamic difficulty adjustment (DDA) has targeted in other domains \cite{ref16,ref17,ref22}, and two recent systems already combine knowledge tracing with bandit- or reinforcement-learning-based difficulty adaptation directly: a hierarchical multi-armed-bandit tutor that uses Bayesian Knowledge Tracing to jointly select the next concept and problem difficulty, validated via simulation with 500 students \cite{ref39}, and a reinforcement-learning difficulty controller coupled to a general ``stealth assessment'' signal in collaborative game-based learning \cite{ref40} -- a line of work tracing back at least to earlier bandit-based content sequencing without full knowledge tracing \cite{ref41}. Neither of the two more recent systems, however, is applied to a serious game whose concept map is grounded in an existing game's own characters and levels, and neither reports a per-concept equity analysis, a calibration check of the tracing estimate against an independently generated ground truth, or a quantified analysis of premature mastery declaration -- the specific gaps this paper addresses (Section 2.3 positions this precedent in full).
...
\begin{itemize}
\item \textbf{An AI layer design coupling knowledge tracing with bandit-based adaptive difficulty} -- a combination with recent precedent in non-game intelligent tutoring \cite{ref39} and, in a more limited form, in game-based learning \cite{ref40} (Section 2.3 distinguishes these directly) -- applied here to a serious game whose concept map is grounded in an existing game's own characters and levels, specified in enough architectural detail (Section 3) to be integrated into that Unity-based game once its client-side implementation is complete.
\item \textbf{A simulation-based evaluation methodology}, in the same spirit as recent simulation-validated knowledge-tracing tutors \cite{ref39} but adapted to a serious-game setting where a complete, deployable game client is often the bottleneck to any evaluation at all, allowing an AI-driven adaptation mechanism to be validated against synthetic learners with heterogeneous skill, learning, and forgetting dynamics before a full game client or real learner population is available.
...
A small number of recent serious-game studies have coupled AI-driven adaptation directly to measured learning outcomes rather than only to raw performance -- for example, AI-generated verbal and visual scaffolding delivered by a non-player character in a physics-themed serious game, shown to affect both learning gains and cognitive load \cite{ref10}; and, more directly relevant here, the GRADES framework, which couples a reinforcement-learning DDA controller with a general ``stealth assessment'' signal in collaborative game-based learning \cite{ref40}. Having since obtained and read GRADES' full text, we can confirm rather than hedge on this point: its evaluated simulation represents each learner with a single scalar skill value drawn uniformly between 1 (novice) and 10 (expert), not a per-concept mastery vector, and success probability is a function of that one scalar against a single difficulty scalar; the paper's own Discussion section states plainly that it ``currently rel[ies] on average performance to evaluate system adaptation'' and lists concept-level (there, ``learning gains'') granularity as future work rather than something already implemented. Its adaptation target is therefore overall game difficulty rather than \textit{which concept to practice next}, so it has no mechanism equivalent to the concept-selection component evaluated in this paper (Section 3.3), and by construction -- not merely by omission -- cannot exhibit or correct the kind of single-concept ``difficulty trap'' reported in Section 4.3.

Outside games, concept-level knowledge tracing has been coupled to bandit-based content selection directly, and this is not a new idea as of 2024--2025: bandit-based sequencing without full knowledge tracing dates to at least 2018 \cite{ref41}, and a hierarchical multi-armed-bandit tutor uses BKT to jointly select the next concept and problem difficulty for a non-game AI-literacy tutoring platform, validated via simulation with 500 students and an ablation contrasting difficulty-agnostic and difficulty-included bandit variants \cite{ref39} -- the closest existing precedent to the mechanism used in this paper, and the reason this paper does not claim the \textit{combination} of knowledge tracing and bandit-based difficulty selection as novel in itself. That system, however, reports only a single aggregate mastery curve pooled across all concepts, not a per-concept equity analysis; its baseline condition is a random question order rather than a fixed, authored progression, so it cannot surface the kind of single-concept trap a badly-matched authored difficulty produces (Section 4.3); and its simulated ground truth is generated by the same class of Markov process its BKT model assumes, which structurally precludes measuring any calibration gap of the kind reported in Section 4.1.

Within educational games specifically, DDA has mostly been applied to generic performance signals (score, completion time) rather than to concept-level mastery estimates, and -- GRADES aside -- the games-adaptivity literature reviewed above has no direct precedent for concept-level knowledge tracing driving difficulty selection. Coupling concept-level knowledge tracing with bandit-based difficulty adaptation -- evaluated specifically for per-concept equity against a fixed, \textit{authored} (not random) baseline, for estimator calibration against an independently generated ground truth, and for the reliability of mastery declarations -- applied to an existing immune-system serious game via the five-concept map introduced in Section 3, is the combination this paper investigates.

It is also worth being explicit that almost all of the DDA-in-education evidence reviewed above, including this paper's own, is simulation-based rather than tested with real players. Where a real-player test has been run, the result is a useful caution rather than a confirmation: a recent quasi-experimental study of Elo-rating-based DDA in a multiplayer educational puzzle game found no significant difference in player experience or gameplay progression between adaptive and fixed-difficulty conditions, attributing this partly to too short a calibration period for the rating system to differentiate players \cite{ref42}. That study uses a simpler adaptation signal than knowledge tracing (an Elo rating, not a per-concept mastery estimate) and a different domain (programming puzzles, not immunology), so it does not directly test this paper's mechanism -- but it is a concrete reminder that a simulation-validated adaptive system's benefits are not guaranteed to reproduce with real players, reinforcing why this paper reports its own findings as simulation-validated only (Section 5.3) rather than assuming they would transfer.

\subsection{Positioning of this work}

Taken together, the literature establishes (i) serious games as an effective but typically non-adaptive medium for immunology education, (ii) knowledge tracing as a mature, interpretable approach to concept-level learner modeling, (iii) DDA/bandit methods as a tractable way to turn a learner model into moment-to-moment gameplay changes, and (iv) at least one recent system that already integrates (ii) and (iii), in a non-game tutoring context \cite{ref39}. This paper's contribution is therefore not the integration itself, which has that precedent, but its application to an existing immune-system serious game's concept and character structure, together with three analyses absent from both that precedent and the game-adaptivity literature reviewed above: a per-concept equity comparison against a fixed, authored (not random) baseline (Section 4.3--4.4); a calibration check of the knowledge-tracing estimate against an independently generated ground truth, which \cite{ref39}'s own simulation design cannot produce (Section 4.1); and a quantified analysis of how often the system's mastery declarations are premature (Section 4.7).
...
\bibitem{ref39} Castleman, B.; Macar, U.; Salleb-Aouissi, A. Hierarchical Multi-Armed Bandits for the Concurrent Intelligent Tutoring of Concepts and Problems of Varying Difficulty Levels. In \textit{Deployable RL Workshop, Reinforcement Learning Conference (RLC 2024)}; arXiv:2408.07208, 2024.
\bibitem{ref40} Rjiba, A.; Cheniti-Belcadhi, L.; Kasperiuniene, J. Adaptive Difficulty and Stealth Assessment in Collaborative Game-Based Learning. \textit{Computer Applications in Engineering Education} \textbf{2025}, \textit{33}, e70102.
\bibitem{ref41} Segal, A.; Ben David, Y.; Williams, J.J.; Gal, K.; Shalom, Y. Combining Difficulty Ranking with Multi-Armed Bandits to Sequence Educational Content. In \textit{Artificial Intelligence in Education (AIED 2018)}, Lecture Notes in Computer Science, vol. 10948; Springer: Cham, Switzerland, 2018.
\bibitem{ref42} Holly, M.; Kassil, A.; Pirker, J. Dynamic Difficulty Adjustment in a Multiplayer Learning Environment: An Exploratory Evaluation of Player Experience. \textit{Multimodal Technologies and Interaction} \textbf{2026}, \textit{10}, 90.
